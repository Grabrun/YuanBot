...
class EmotionState(BaseModel):
    ...
    dominance: str = "medium"  # "low" | "medium" | "high"
    needs_immediate_comfort: bool = False
    confidence: float = 0.8  # 情感分析的置信度
...
class EmotionRecord(BaseModel):
    ...
    # 上下文
    trigger_text: str  # 触发情感分析的文本
    context_summary: str | None = None  # 对话上下文摘要

    # 分析结果
    analysis_method: str = "rule_based"  # "rule_based" | "model_based" | "hybrid"
    raw_scores: dict[str, float] = Field(default_factory=dict)  # 原始情感分数
...
class EmotionTrend(BaseModel):
    ...
    valence_ratio: dict[str, float] = Field(
        default_factory=dict
    )  # {"positive": 0.6, "negative": 0.3, "neutral": 0.1}

    # 趋势指标
    mood_stability: float = 0.5  # 情绪稳定性 0.0 ~ 1.0
    emotional_variety: float = 0.5  # 情感多样性 0.0 ~ 1.0
    comfort_need_frequency: float = 0.0  # 需要安慰的频率
...
class EmotionPattern(BaseModel):
    ...
    user_id: str
    pattern_type: str  # "temporal" | "situational" | "topic_based"

    # 模式描述
    description: str
    trigger_conditions: dict[str, Any] = Field(default_factory=dict)
    typical_response: EmotionState
