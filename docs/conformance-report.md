---
title: YuanBot 设计文档符合度审查报告 v37
description: YuanBot v1.5.0 设计文档与代码实现的符合度审查，覆盖十大核心系统
---

# 🌸 YuanBot 设计文档符合度审查报告 v37

**审查日期**: 2026-06-13
**审查范围**: docs/ 目录下 17 份设计文档 vs src/ + configs/ + tests/ + webui/ 实际代码
**项目版本**: v1.5.0
**上次审查**: v36 (2026-06-11),总体 ~100%
...
| 功能 | 状态 | 缺失说明 |
|------|------|----------|
| 本地意图模型 (bert-base) | ✅ 已实现 | `MLIntentClassifier` + `SklearnIntentClassifier` + `create_intent_classifier` 工厂函数，支持 ONNX 和 sklearn 双模型，CLI `yuanbot intent` 命令 |
...
本次审查在总体符合度已达 ~100% 的情况下，聚焦能力调用编排器的工具执行并行化优化，将多个独立工具调用从串行执行改为并发执行，并修复了 1 项 PERF lint 问题。

### v35 性能优化

1. **工具调用并行执行** — `services/capability_orchestrator.py` `execute_tool_loop()` 将多个工具调用从串行 `for` 循环改为 `asyncio.gather()` 并行执行。LLM 单次响应中返回的多个 tool_calls 互相独立，可并发执行以减少总延迟。新增 `_execute_single_tool()` 辅助方法封装单个工具的权限检查+执行逻辑。当 LLM 返回 N 个工具调用时，延迟从 O(N*T) 降至 O(T)，T 为单个工具执行时间。
2. **记忆冲突检测列表推导式优化** — `memory/manager.py` `_find_similar_facts_by_entities()` 中 category 级补充扫描的 for+append 改为 `nodes.extend(...)` 生成器表达式，消除逐次 Python append 调用开销。

### 修改的源文件

- `src/yuanbot/services/capability_orchestrator.py` — 新增 `import asyncio`，工具调用并行执行，新增 `_execute_single_tool()` 方法
- `src/yuanbot/memory/manager.py` — `_find_similar_facts_by_entities()` list.extend 优化

### 代码质量

- Ruff lint (src/): All checks passed
- PERF lint: 0 issues
- 测试: 1412 passed, 14 warnings
...
本次审查在总体符合度已达 ~100% 的情况下，修复了 4 项 B905 lint 警告、1 个只读操作错误递增交互计数的正确性 Bug，并实现了会话存储的两项性能优化。

### v36 Bug 修复

1. **get_task_priority 交互计数膨胀修复** — `proactive/strategy.py` `get_task_priority()` 原先调用 `get_or_create_user_profile()`，该方法会原子递增 `total_interactions` 计数。在只读优先级查询场景下（"获取任务优先级"），不应递增交互计数。改为调用 `_get_user_profile_readonly()`，与 v33 中 `should_send()` 的修复保持一致。

### v36 Lint 修复

1. **B905 zip() without strict=** — 4 处 `zip()` 调用添加 `strict=False` 参数：
   - `proactive/event_engine.py` — 情感趋势批量查询结果迭代
   - `proactive/event_engine.py` — 天气检查结果迭代
   - `proactive/event_engine.py` — 特殊日期设置结果迭代
   - `tts/manager.py` — TTS 引擎状态查询结果迭代

### v36 性能优化

1. **会话消息计数器** — `auth/store.py` `ConversationStore` 新增 `_total_message_count` 运行计数器，`add_message()` 原子递增，`delete_conversation()` 原子递减，`total_message_count` 属性直接返回计数器值。原先每次调用都需遍历所有会话的所有消息列表求 `len()` 和（O(N) 遍历），优化后 O(1) 直接返回。
2. **会话列表排序缓存** — `ConversationStore` 新增 `_sorted_conversations_cache` 字典和 `_sort_cache_dirty` 脏标记集合。`list_conversations()` 在缓存有效时直接返回已排序列表（O(1)），仅在脏标记存在时重新排序。`add_message()`、`create_conversation()`、`delete_conversation()` 通过 `_sort_cache_dirty.add(user_id)` 使缓存失效。

### 修改的源文件

- `src/yuanbot/proactive/strategy.py` — `get_task_priority()` 改用 `_get_user_profile_readonly()`
- `src/yuanbot/proactive/event_engine.py` — 3 处 `zip()` 添加 `strict=False`
- `src/yuanbot/tts/manager.py` — 1 处 `zip()` 添加 `strict=False`
- `src/yuanbot/auth/store.py` — `_total_message_count` 计数器 + `_sorted_conversations_cache` 排序缓存
- `tests/test_proactive/test_strategy.py` — 更新 2 个测试用例 mock `_get_user_profile_readonly`

### 代码质量

- Ruff lint (src/): All checks passed
- B905 lint: 0 issues (was 4)
- PERF lint: 0 issues
- 测试: 1412 passed, 14 warnings
...
| 系统 | v35 | v36 | 变化 |
|------|------|------|------|
| **总体** | **~100%** | **~100%** | — (Bug 修复 + 性能优化 + Lint 修复，无功能变更) |
...
本次审查在总体符合度已达 ~100% 的情况下，清理了全部 12 个 RUF100 悬浮 `noqa` 指令和 6 个 F 类 lint 问题（未使用的导入和变量），并优化了依赖检查的 import 模式。

### v37 Lint 修复

1. **RUF100 悬浮 noqa 清理 (12 处)** — 移除不再需要的 `noqa` 指令，涉及 5 个源文件。替换裸 import 为 `importlib.util.find_spec` 语义等价写法
   - `cli.py` (3 处) — 移除 `# noqa: F401`
   - `persona/default.py` (3 处) — 移除 `# noqa: F841`，删除未使用的 `stage_config` 赋值
   - `persona/engines/decision_plugin.py` (2 处) — 移除 `# noqa: B027`
   - `proactive/trigger.py` (2 处) — 移除 `# noqa: B027`
   - `tts/edge_tts_adapter.py` (1 处) — 移除 `# noqa: F401`
   - `tts/openai_tts_adapter.py` (1 处) — 移除 `# noqa: F401`

2. **F401 未使用导入修复 (6 处)** — 将裸 `import` 替换为 `importlib.util.find_spec()` 检查，语义不变但消除 F401 警告

3. **F841 未使用变量修复 (1 处)** — `persona/default.py` `get_behavior_rules()` 删除未使用的 `stage_config = self.get_stage_config()` 赋值

### v37 代码优化

1. **import 集中化** — `cli.py` 将 `import importlib.util` 从 3 个 try 块中提升到文件顶部的模块级导入，遵循 PEP 8 导入规范

### 修改的源文件

- `src/yuanbot/cli.py` — F401 修复 + `importlib.util` 模块级导入
- `src/yuanbot/persona/default.py` — F841 修复 + RUF100 noqa 清理
- `src/yuanbot/persona/engines/decision_plugin.py` — RUF100 noqa 清理
- `src/yuanbot/proactive/trigger.py` — RUF100 noqa 清理
- `src/yuanbot/tts/edge_tts_adapter.py` — F401 修复 + RUF100 noqa 清理
- `src/yuanbot/tts/openai_tts_adapter.py` — F401 修复 + RUF100 noqa 清理

### 代码质量

- Ruff lint (src/): **All checks passed**（RUF100: 0 issues, F: 0 issues）
- PERF/SIM/C4/B lint: 0 issues
- 测试: **1412 passed**, 14 warnings
...
|--------|------|----------|------|
| — | **全部 P3 项目已完成** 🎉 | — | — |
| P3 | 人格商店 WebUI | 2-3 天 | 前端功能 ✅ v13 |
| P3 | 记忆图谱可视化 (ECharts) | 2-3 天 | 前端功能 ✅ v13 |
| P3 | 流式播放同步 (WebSocket) | 1-2 天 | 前端集成 ✅ v13 |
| P3 | 日志聚合 (Loki + Grafana) | 2-3 天 | 运维增强 ✅ v13 |
| P3 | yuanbot-testkit 测试框架 | 3-5 天 | 开发工具 ✅ v38 |
| P3 | 多语言文档站 (VitePress) | 3-5 天 | 文档 ✅ v38 |
| P3 | 社区贡献流水线 (PR→CI→上架) | 2-3 天 | 流程工具 ✅ v38 |

---

## v38 更新摘要

本次审查完成了剩余的全部 3 个 P3 增强项：yuanbot-testkit 测试框架、VitePress 多语言文档站、社区贡献 CI/CD 流水线。至此所有 P3 项目均已完成。

### v38 新增实现

1. **yuanbot-testkit 测试框架** — `src/yuanbot_testkit/` 包：`MockCore` 模拟 AI 对话/嵌入/记忆/工具执行，`TestAdapter` 模拟通道消息收发；提供 8 个 pytest fixtures；41 个测试覆盖；通过 `pip install -e ".[test]"` 安装
2. **VitePress 多语言文档站** — `docs-vitepress/`：中文完整内容（首页/指南/API/贡献），英文/日文 WIP 占位，`npx vitepress build` 成功，GitHub Actions 自动部署到 GitHub Pages
3. **社区贡献流水线** — 增强 `ci.yml`（新增 extension-validate 作业）、新增 `pr-review.yml`（PR 自动审查 + 标签）、新增 `publish.yml`（自动发布 Release + Docker）；`CONTRIBUTING.md` 全面重写

### 新增源文件

- `src/yuanbot_testkit/__init__.py`
- `src/yuanbot_testkit/mock_core.py` — MockCore (350行)
- `src/yuanbot_testkit/mock_adapter.py` — TestAdapter (280行)
- `src/yuanbot_testkit/fixtures.py` — 8个 pytest fixtures
- `tests/test_testkit/test_mock_core.py` — 22个测试
- `tests/test_testkit/test_mock_adapter.py` — 14个测试
- `tests/test_testkit/test_fixtures.py` — 7个测试
- `tests/test_testkit/conftest.py`
- `docs-vitepress/` — VitePress 三语言文档站 (25个md文件)
- `.github/workflows/pr-review.yml` — PR 自动审查
- `.github/workflows/publish.yml` — 自动发布
- `.github/workflows/docs-vitepress-deploy.yml` — 文档站部署

### 修改源文件

- `pyproject.toml` — 添加 `[test]` optional-dependencies + `src/yuanbot_testkit` wheel packages
- `.github/workflows/ci.yml` — 新增 extension-validate 作业
- `CONTRIBUTING.md` — 全面重写 (386行，含 PR 流程/扩展开发/贡献清单)

### 代码质量

- Ruff lint (src/ + tests/): **All checks passed**
- 测试: **1453 passed** (+41 testkit 测试)
